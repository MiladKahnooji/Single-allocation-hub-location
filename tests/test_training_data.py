...
def test_graphs_follow_demand_coverage_and_nearest_node_semantics() -> None:
    ...
    assert np.array_equal(production, [[0, 1, 0], [0, 0, 1], [1, 0, 0]])
    assert np.array_equal(attraction, [[0, 0, 1], [1, 0, 0], [0, 1, 0]])
    assert np.array_equal(spatial, [[0, 1, 0], [1, 0, 0], [0, 1, 0]])
...
def test_tiny_exact_labels_are_marked_proven_optimal() -> None:
    data = generate_synthetic_hub_data(
        seed=3, node_count=4, p=1, time_limit=10.0, label_method="exact"
    )
...
def test_cab25_feature_pipeline_smoke() -> None:
    ...
    assert data.features.shape == (25, 7)
